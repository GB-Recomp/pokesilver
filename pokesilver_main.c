...
#include "assets_manifest_pokesilver.h"
extern uint8_t pokesilver__rom_data[];
extern const size_t pokesilver__rom_size;
static const GBGameAssets GB_POKESILVER_GAME = {
    .game_id = "pokesilver",
    .rom_filename = "roms/pokesilver.gbc",
    .rom_data = pokesilver__rom_data,
    .rom_size = 2097152u,
    .expected_sha1 = { 0x49, 0xb1, 0x63, 0xf7, 0xe5, 0x77, 0x02, 0xbc, 0x93, 0x9d, 0x64, 0x2a, 0x18, 0xf5, 0x91, 0xde, 0x55, 0xd9, 0x2d, 0xae },
    .manifest = POKESILVER_ASSETS_MANIFEST,
    .manifest_count = POKESILVER_ASSETS_MANIFEST_COUNT,
};
...
int pokesilver_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *pokesilver_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "pokesilver");
        gb_context_set_save_id(interpreted_ctx, "pokesilver");
        pokesilver_init(generated_ctx);
        pokesilver_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "pokesilver");
    ...
    if (!gb_chdir_to_exe_dir()) {
        fprintf(stderr, "[pokesilver] chdir to exe dir failed\n");
        return 1;
    }
    if (!gb_load_assets(&GB_POKESILVER_GAME)) {
        fprintf(stderr, "[pokesilver] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_POKESILVER_GAME.game_id);
#endif
    pokesilver_init(ctx);
    int exit_code = 0;
